...
public class AnimalNameChecker {
    ...
    private static final String MESSAGE_FOR_INVALID_LENGTH =
        String.format("The length of name must be greater than %d and less than %d", MIN_NAME_LENGTH, MAX_NAME_LENGTH);
    private static final String MESSAGE_FOR_INVALID_SYMBOLS = "The name can contain only latin letters and spaces";
    private static final String MESSAGE_FOR_CONTAINING_SPACES_AT_BEGIN_OF_END =
        "The name must not contain space characters at the beginning or end";
    private static final String MESSAGE_FOR_INVALID_REGISTER =
    ...
    static boolean isContainOnlyAllowedSymbols(String name) {
        return ALLOWED_SYMBOLS_PATTERN.matcher(name).matches();
    }

    static boolean isContainSpacesAtBeginningOrEnd(String name) {
        return name.charAt(0) == ' ' || name.charAt(name.length() - 1) == ' ';
    }

    static boolean isValidRegister(String name) {
